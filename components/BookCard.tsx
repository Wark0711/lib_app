import Link from "next/link";
import { BookCover } from "./BookCover";
import { cn } from "@/lib/utils";
import Image from "next/image";
import { Button } from "./ui/button";

export function BookCard({ id, title, genre, coverColor, coverUrl }: Book) {
    const isLoanedBook = false
    const newTitle = title?.length > 15 ? `${title?.substring(0, 15)}...` : title
    return (
        <>
            <li className={cn(isLoanedBook && 'xs:w-52 w-full')}>
                <Link href={`/books/${id}`} className={cn(isLoanedBook && 'w-full flex-col items-center')}>
                    <BookCover coverColor={coverColor} coverUrl={coverUrl} />

                    <div className={cn('mt-4', !isLoanedBook && 'xs:max-w-40 max-w-28')}>
                        <p className="book-title">{newTitle}</p>
                        <p className="book-genre">{genre}</p>
                    </div>

                    {
                        isLoanedBook
                            ? <div className="mt-3 w-full">
                                <div className="book-loaned">
                                    <Image src={'/icons/calendar.svg'} alt="calendar" width={18} height={18} className="object-contain" />
                                    <p className="text-light-100">11 days left to return</p>
                                </div>

                                <Button variant={'default'} className="book-btn">Download Receipt</Button>
                            </div>
                            : <></>
                    }
                </Link>
            </li>
        </>
    )
}